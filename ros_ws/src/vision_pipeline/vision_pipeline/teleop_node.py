...
HEAD_STEP = 0.1          # angular increment per key press
HEAD_PAN_MIN, HEAD_PAN_MAX = -1.24, 1.24
HEAD_TILT_MIN, HEAD_TILT_MAX = -0.98, 0.79

TORSO_STEP = 0.02         # meter increment per key press
TORSO_MIN, TORSO_MAX = 0.0, 0.35

MOVE_DURATION_SEC = 1     # time to reach the target position (head/torso)
...
class KeyboardTeleop(Node):
    def __init__(self):
        ...
        self.torso_pos = 0.15  # approximate initial position
...
def get_key(settings, timeout=0.1):
    ...
    if rlist:
        ...
        if key == '\x1b':  # possible arrow sequence: ESC [ A/B/C/D
...
def main(args=None):
    ...
    # Spin the node in a separate thread so keyboard reading is not blocked
    ...
    try:
        ...
        while rclpy.ok():
            ...
            # --- Head: arrows ---
            elif key == 'i':   # up arrow -> tilt up
                node.head_tilt += HEAD_STEP
                node.send_head()
            elif key == 'p':   # down arrow -> tilt down
                node.head_tilt -= HEAD_STEP
                node.send_head()
            elif key == 'j':   # right arrow -> pan right
                node.head_pan -= HEAD_STEP
                node.send_head()
            elif key == 'l':   # left arrow -> pan left
                node.head_pan += HEAD_STEP
                node.send_head()
            ...
            # --- Exit ---
            elif key == '\x03' or key == '\x1b':  # Ctrl+C or pure ESC
                break
