...
| `models_comparison.py` | `yolo_models_comparison` | Confronto visivo rapido tra tutti i modelli dello sweep: fa inferenza con i pesi (`best.pt`) di ogni run su una singola immagine del test set (default: `frame_010127`, caso difficile con coca/pringles/biscotti/aruco ammassati e sovrapposti — buon test per l'occlusion handling), salvando un'immagine annotata per modello in `models/runs/models_comparison/<nome_immagine>/<run_id>.png`. Salta automaticamente le run senza `best.pt` (interrotte/incomplete). Nota: la confidenza su una singola immagine non e' una metrica di performance affidabile — per confrontare i modelli sul serio usa `models/wandb/all_sweeps_summary.csv` (mAP50-95 sull'intero test set). |
