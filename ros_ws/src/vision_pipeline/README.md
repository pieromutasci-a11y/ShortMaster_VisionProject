...
| `models_comparison.py` | `yolo_models_comparison` | Grafici riassuntivi di confronto tra le run dello sweep, **dalle metriche vere** (`all_sweeps_summary.csv`, mAP50-95 calcolata da `train_sweep.py` sull'intero test set — non serve girare inferenza su nessuna immagine). Salvati in `models/runs/models_comparison/plots/`: classifica di tutte le run sulla metrica ottimizzata (`map5095_grasp_ranking.png`), la run scelta (`HIGHLIGHT_RUN_ID`) vs media dello sweep per singola classe (`per_class_vs_average.png`), e metrica ottimizzata vs metrica generale per verificare che il vantaggio non sia un artefatto (`grasp_vs_all.png`). |
