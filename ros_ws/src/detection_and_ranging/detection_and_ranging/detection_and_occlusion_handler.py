...
class RtObjectDetectionNode(Node):
    ...
    def estimate_position_light_mode(self, detection_results, depth_image, image_width, image_height,
        ...
        """
        Simple/fast position estimate: takes the depth at the centre pixel of
        the target's bounding box.

        Only appropriate when the target is known to never be occluded by
        another tracked object in the scene (e.g. a fixed, uncluttered setup) —
        it does not correct for background pixels or other objects leaking
        into the bounding box. For occluded scenes, use the robust mode
        (estimate_position_with_occlusion_handling) instead.
        """
        ...
        for bounding_box in detection_results.boxes:
            ...
            if position is None:
                self.get_logger().warn("DEBUG light: depth non valido al centro bbox della coca-cola", throttle_duration_sec=2.0)
                continue
    ...
    def estimate_position_with_occlusion_handling(self, detection_results, depth_image,
        ...
        if TARGET_CLASS_NAME not in clean_median_depth:
            self.get_logger().warn(
                "Not enough clean pixels to estimate coca-cola depth for ordering.",
                throttle_duration_sec=5.0
            )
            return

        # --- Step 3.5: refuse to guess when a target-involving ordering is shaky ---
        # If either side of a pair that geometrically overlaps the target has
        # too few clean pixels (absolute count or fraction of its own bbox),
        # we can't trust who is really in front. Getting this wrong means
        # either leaving the target contaminated or wrongly excluding valid
        # target pixels — either can lead to a bad grasp, so we'd rather skip
        # the frame. Pairs that don't overlap the target are irrelevant here:
        # Step 5 never excludes anything for them regardless of ordering.
        target_pixel_count, target_coverage_ratio = clean_pixel_stats[TARGET_CLASS_NAME]
        if target_coverage_ratio < MIN_ORDERING_COVERAGE_RATIO:
            self.get_logger().warn(
                f"Coca-cola's own clean-pixel coverage too low to trust any "
                f"occlusion ordering involving it ({target_coverage_ratio:.0%}).",
                throttle_duration_sec=5.0
            )
            return
        ...
        for class_name in detected_boxes:
            if class_name == TARGET_CLASS_NAME:
                continue
            overlap_key = frozenset((class_name, TARGET_CLASS_NAME))
            if overlap_key not in overlap_regions:
                continue  # doesn't overlap the target, ordering with it is irrelevant

            other_pixel_count, other_coverage_ratio = clean_pixel_stats[class_name]
            if other_pixel_count < MIN_PIXELS_FOR_ORDERING or other_coverage_ratio < MIN_ORDERING_COVERAGE_RATIO:
                self.get_logger().warn(
                    f"Ordering between coca-cola and {class_name} is too "
                    f"unreliable to trust ({other_pixel_count} px, "
                    f"{other_coverage_ratio:.0%} coverage) — skipping this frame "
                    f"rather than risking a bad grasp.",
                    throttle_duration_sec=5.0
                )
                return
        ...
        target_depth = clean_median_depth[TARGET_CLASS_NAME]
        target_x1, target_y1, target_x2, target_y2 = detected_boxes[TARGET_CLASS_NAME]

        # --- Step 5: exclude pixels occluded by objects genuinely closer than the target ---
        exclusion_mask = np.zeros((target_y2 - target_y1, target_x2 - target_x1), dtype=bool)
        for class_name, depth_value in ordered_by_depth:
            if class_name == TARGET_CLASS_NAME:
                break  # remaining objects in the ordering are farther, they can't occlude the target

            if (target_depth - depth_value) <= DEPTH_ORDER_EPSILON_M:
                # Numerically indistinguishable from the target's own depth —
                # treat as "not reliably closer" to avoid flip-flopping on
                # floating-point/render noise between frames. Any genuine
                # depth difference beyond this tiny tolerance is treated as
                # real occlusion (no physical minimum gap is required for a
                # nearer surface to block a farther one).
                continue

            overlap_key = frozenset((class_name, TARGET_CLASS_NAME))
            if overlap_key not in overlap_regions:
                continue  # closer, but its box doesn't actually overlap the target's

            ox1, oy1, ox2, oy2 = overlap_regions[overlap_key]
            # Translate from image coordinates to coordinates local to the
            # target's own bounding box, so it can index into it directly.
            exclusion_mask[oy1 - target_y1:oy2 - target_y1, ox1 - target_x1:ox2 - target_x1] = True

        # --- Step 6: combine occlusion exclusion with the depth-validity filter ---
        target_depth_region = depth_image[target_y1:target_y2, target_x1:target_x2]
        keep_mask = ~exclusion_mask

        # Combined as a 2D mask (not just flattened arrays) so Step 9 can
        # later slice a single row directly when searching for grasp-circle
        # points, reusing the exact same mask Step 8 already relies on.
        target_valid_mask = keep_mask & self.valid_depth_mask(target_depth_region)

        # np.where gives the (row, col) of every kept pixel in the same order
        # that boolean indexing extracts their depth values below — this keeps
        # pixel coordinates and depth values in sync for the step 8 lookup.
        rows_local, cols_local = np.where(target_valid_mask)
        clean_depth_values = target_depth_region[target_valid_mask]

        # --- Step 7: sanity check — enough clean pixels left to trust the estimate? ---
        original_area = (target_x2 - target_x1) * (target_y2 - target_y1)
        ...
        if clean_depth_values.size < MIN_VALID_PIXELS or coverage_ratio < MIN_COVERAGE_RATIO:
            self.get_logger().warn(
                f"Coca-cola too occluded to trust the position estimate "
        ...
        # First restrict to pixels within a small tolerance of the median
        # depth — on a cylindrical object these tend to be physically close
        # together (the portion of the surface facing the camera head-on),
        # so averaging within this restricted set doesn't recreate the
        # "chimeric point" problem (that came from combining independent
        # statistics over the WHOLE, spatially spread-out pixel set).
        ...
        if np.any(near_median_mask):
            # Step A: find a real, guaranteed-valid starting pixel close to
            # the cluster's centroid. The plain mean can fall outside the
            # visible shape on non-convex silhouettes (e.g. an "L" left
            # after occlusion — see conversation), so instead of sampling
            # the mean directly, pick the nearest actual member of the
            # cluster to it.
            ...
            # Step B: push this starting pixel away from the occlusion
            # boundary with two sequential contiguous-segment cuts — fix
            # the row, take the midpoint of the column-run containing the
            # start; then fix THAT NEW column and take the midpoint of the
            # row-run containing the start. Chaining the second cut off the
            # new column (not the old one) is what guarantees the result
            # always lands on a valid pixel, for any mask shape — no
            # validity check or fallback needed afterwards.
            col_lo, col_hi = self.contiguous_valid_segment(target_valid_mask[start_row, :], start_col)
            centred_col = round((col_lo + col_hi) / 2)

            row_lo, row_hi = self.contiguous_valid_segment(target_valid_mask[:, centred_col], start_row)
            centred_row = round((row_lo + row_hi) / 2)

            final_depth = float(target_depth_region[centred_row, centred_col])
            final_pixel_x = centred_col + target_x1
            final_pixel_y = centred_row + target_y1
        else:
            # Rare fallback: no pixel fell within the tolerance band (e.g.
            # coarsely quantised depth) — degrade to the old single-pixel
            # behaviour rather than crashing on an empty mean.
            closest_idx = np.argmin(np.abs(clean_depth_values - median_depth))
            final_depth = float(clean_depth_values[closest_idx])
            final_pixel_x = int(cols_local[closest_idx]) + target_x1
            final_pixel_y = int(rows_local[closest_idx]) + target_y1

                # --- Back-projection (pinhole model), same formula as light mode ---
        point_z = final_depth
        ...
        # Publish the anchor point on the same single-point topic used
        # before the multi-point message existed, so any consumer still
        # relying on it keeps working unchanged.
        self.publish_object_position(point_x, point_y, point_z, header)

        # --- Step 9: sample NUM_GRASP_CIRCLE_POINTS points at ~constant height ---
        start_row_local = int(round(final_pixel_y)) - target_y1
        start_col_local = centred_col
        grasp_circle_points = self.select_grasp_circle_points(
            target_depth_region, target_valid_mask, start_row_local, start_col_local,
            target_x1, target_y1,
            focal_length_x, focal_length_y, principal_point_x, principal_point_y
        )

        if grasp_circle_points is None:
            # No row within MAX_ROW_SEARCH_OFFSET had enough valid columns —
            # degrade to the single Step 8 point repeated 3 times rather
        ...
        tracked_objects_msg.objects = [
            TrackedObjectPoints(
                ...
                points=grasp_circle_points,
            )
        ]
        self.tracked_objects_pub.publish(tracked_objects_msg)

        # --- RViz visualisation: the 3 points as a small sphere list ---
        marker_msg = Marker()
        ...
        marker_msg.points = grasp_circle_points
