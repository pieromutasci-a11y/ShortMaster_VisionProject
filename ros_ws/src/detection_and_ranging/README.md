...
| `center_computation_all.py` | `center_computation_all` | Versione multi-oggetto di `center_computation.py`: stessa correzione geometrica (raggio noto, calcolata in `base_footprint`), ma per classe — un raggio diverso per ciascuna (`RADIUS_BY_CLASS`: 0.04 per le lattine, 0.03 per i biscotti come approssimazione mezzo-spessore, 0.0 per `dinner table` cioe' nessuna correzione, non ha un asse con senso fisico). Topic per classe (`centers_all/<classe>_center`, `centers_all/<classe>_axis`), con id marker e colore diversi per classe e per istanza nello stesso frame. |
